...
export async function handleGetOwners(): Promise<string[]> {
  ...
  if (fs.existsSync(rawStatementFilesDir)) {
    return fs
      .readdirSync(rawStatementFilesDir)
      .filter((f) =>
        fs.statSync(path.join(rawStatementFilesDir, f)).isDirectory(),
      );
  }
  return [];
}
...
export async function handleImportFile(
  ...
  if (fs.existsSync(categoryCsvPath) && fs.existsSync(transactionsCsvPath)) {
    const categories = fs
      .readFileSync(categoryCsvPath, "utf-8")
      .split("\n")
      .filter((l) => l.trim());
    const transactions = fs
      .readFileSync(transactionsCsvPath, "utf-8")
      .split("\n")
      .filter((l) => l.trim());
...
export async function handleSaveCategory(
...
export async function handleBulkSaveMetadata(
  updates: { transactionHash: string; category?: string; tags?: string }[],
): Promise<{ success: boolean }> {
  const { dataDir } = getPaths();
  const categoryCsvPath = path.join(
    dataDir,
  ...
  for (const update of updates) {
    const existing = rowsMap.get(update.transactionHash) || {
      category: "",
      tags: "",
    };
...
export async function handleGetMetadata(): Promise<{
  tags: any[];
  categories: any[];
}> {
  const { dataDir } = getPaths();
  ...
  if (fs.existsSync(tagsPath)) {
    try {
      ...
      console.error("Error parsing tags meta", e);
    }
  } else {
    ...
    try {
      ...
      DEFAULT_CATEGORIES.forEach((defCat) => {
        ...
        if (index === -1) {
          ...
          if (
            categories[index].color !== defCat.color ||
            !categories[index].isDefault
          ) {
            categories[index] = {
              ...categories[index],
              color: defCat.color,
      ...
      console.error("Error parsing categories meta", e);
    }
  } else {
...
export async function handleSaveMetadata(
  ...
  const filePath = path.join(
    dataDir,
    type === "tags" ? "meta-tags.json" : "meta-categories.json",
  );
...
export async function handleBackupCategories(): Promise<{
  success: boolean;
  fileName?: string;
  error?: string;
}> {
  ...
  if (fs.existsSync(categoryCsvPath)) {
    ...
    const backupCsvPath = path.join(
      backupDir,
      `monthly-transactions-category-${timestamp}-bkp.csv`,
    );
    fs.copyFileSync(categoryCsvPath, backupCsvPath);

    // Backup Meta Tags
    if (fs.existsSync(tagsJsonPath)) {
      const backupTagsPath = path.join(
        backupDir,
        `meta-tags-${timestamp}-bkp.json`,
      );
    ...
    if (fs.existsSync(catsJsonPath)) {
      const backupCatsPath = path.join(
        backupDir,
        `meta-categories-${timestamp}-bkp.json`,
      );
...
export async function handleGetBackupInfo(): Promise<{
  count: number;
